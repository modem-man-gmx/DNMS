 ...
 *                                                                      *
 *  Version for DNMS Sensor with 1/3 Octave Values for InfluxDB         *
 *  1/ Octave Values are not send to Sensor.Community                   *                                     *
 *  if transmission to Sensor.Community is set,                         * 
 *  than measurement interval is set to 150 seconds                     *
 *  A 2nd measurement interval is supported for transmission to         *
 *  an InfluxDB                                                         *   
 *                                                                      *
 *                                                                      *
 ************************************************************************
 ...
 *    Copyright (C) 2022-2024  Helmut Bitter                            *
 *                                                                      *
...
#if defined(ESP8266)
#include <LittleFS.h>  // as alternative to SPIFFS
 ...
 * MQTT transmission                                             *
 *****************************************************************/
WiFiClient wifi_client_for_mqtt;
 ...
 ******************************************************************/
#define sc_sending_intervall_ms 150000
...
namespace cfg {
  ...
  unsigned sending_intervall_ms = sc_sending_intervall_ms;
  unsigned i2c_gpio_scl = 2;
  unsigned i2c_gpio_sda = 0;
  unsigned dnms_2nd_interval_time = 60000;
  unsigned dnms_2nd_threshold = 60;
  unsigned dnms_2nd_number_measurements = 120;
  unsigned dnms_2nd_gpio_pin = 16;
  bool powersave = POWERSAVE;
  ...
  bool start_on_full_minute = START_ON_FULL_MINUTE;
  bool start_on_full_hour = START_ON_FULL_HOUR;
  bool sensor_cyclic_restart = SENSOR_CYCLIC_RESTART;
  unsigned sensor_cyclic_restart_time = 24;
  ...
  bool scd4x_read = SCD4X_READ;
  bool scd4x_low_power = SCD4X_LOW_POWER;
  bool ds18b20_read = DS18B20_READ;
  bool dnms_read = DNMS_READ;
  bool dnms_read_active = false;
  bool dnms_read_Z = DNMS_READ_Z;
  bool dnms_read_Z_active = false;
  bool dnms_read_spectrum = DNMS_READ_SPECTRUM;
  bool dnms_read_spectrum_Z = DNMS_READ_SPECTRUM_Z;
  bool dnms_read_spectrum_active = false;
  bool dnms_read_spectrum_Z_active = false;
  bool dnms_2nd_interval = DNMS_2nd_INTERVAL;
  bool dnms_2nd_interval_Z = DNMS_2nd_INTERVAL_Z;
  bool dnms_2nd_read_spectrum = DNMS_2nd_READ_SPECTRUM;
  bool dnms_2nd_read_spectrum_Z = DNMS_2nd_READ_SPECTRUM_Z;
  bool dnms_2nd_interval_active = false;
  bool dnms_2nd_interval_Z_active = false;
  bool dnms_2nd_read_spectrum_active = false;
  bool dnms_2nd_read_spectrum_Z_active = false;
  bool dnms_2nd_switch_output = DNMS_2nd_SWITCH_OUTPUT;
  char dnms_correction[LEN_DNMS_CORRECTION] = DNMS_CORRECTION;
  bool dnms_ICS43434 = DNMS_ICS43434;
  bool dnms_IM72D128 = DNMS_IM72D128;
  ...
  char user_mqtt[LEN_USER_MQTT] = USER_MQTT;
  char pwd_mqtt[LEN_PASS_MQTT] = PWD_MQTT;
  char measurement_name_mqtt[LEN_MEASUREMENT_NAME_MQTT];
  char mqtt_main_topic[LEN_MQTT_MAIN_TOPIC];
  bool id_as_mqtt_sub_topic = ID_AS_MQTT_SUB_TOPIC;
  bool tls_mqtt = TLS_MQTT;
  ...
  char ntp_1[LEN_NTP_1];
  char ntp_2[LEN_NTP_2];
  ...
  void initNonTrivials(const char *id) {
    ...
    strcpy_P(measurement_name_mqtt, MEASUREMENT_NAME_MQTT);
    strcpy_P(ntp_1, NTP_SERVER_1);
    strcpy_P(ntp_2, NTP_SERVER_2);
...
float last_value_dnms_laeq_freq_spec[31] = { -1.0 };
float last_value_dnms_laeq_2nd = -1.0;
float last_value_dnms_la_min_2nd = -1.0;
float last_value_dnms_la_max_2nd = -1.0;
float last_value_dnms_laeq_freq_spec_2nd[31] = { -1.0 };
time_t dnms_now;
time_t dnms_2nd_now;
bool send_2nd_dnms = false;
uint16_t dnms_2nd_count = 0;
float last_value_dnms_lzeq = -1.0;
float last_value_dnms_lz_min = -1.0;
float last_value_dnms_lz_max = -1.0;
float last_value_dnms_lzeq_freq_spec[31] = { -1.0 };
float last_value_dnms_lzeq_2nd = -1.0;
float last_value_dnms_lz_min_2nd = -1.0;
float last_value_dnms_lz_max_2nd = -1.0;
float last_value_dnms_lzeq_freq_spec_2nd[31] = { -1.0 };
bool dnms_error = false;
uint16_t DNMS_error_continous_count = 0;
float dnms_corr_value;

/*****************************************************************
 * Variables for temperature correction                          *
 *****************************************************************/
float temp_corr_value;
...
bool send_now = false;
bool send_dnms_2nd_interval = false;
...
uint16_t last_value_SCD30_firmware = 0;
float last_value_SCD4X_T = -128.0;
float last_value_SCD4X_H = -1.0;
uint16_t last_value_SCD4X_CO2 = 0;
...
String last_value_DNMS_version;
String last_value_SPS30_version;
String last_value_SCD4X_serialNumber;
String last_value_SEN5X_serialNumber;
String last_value_SEN5X_type;
String last_value_SEN5X_hw;
String last_value_SEN5X_sw;

unsigned long SDS_error_count;
unsigned long NPM_error_count;
unsigned long IPS_error_count;
unsigned long WiFi_error_count;
unsigned long DNMS_error_count;
unsigned long SCD30_read_error_counter;
unsigned long SCD4X_read_error_counter;
...
void printModuleVersions() {
  ...
  if (error) {
    ...
    Debug.print(", ");
    last_value_SEN5X_sw = String(firmwareMajor) + String(".") + String(firmwareMinor);

    Debug.print("Hardware: ");
    Debug.print(hardwareMajor);
    Debug.print(".");
    Debug.println(hardwareMinor);
    last_value_SEN5X_hw = String(hardwareMajor) + String(".") + String(hardwareMinor);
  }
}
...
void printSerialNumber() {
  ...
  if (error) {
    ...
    last_value_SEN5X_serialNumber = String((char *)serialNumber);
  }
}
...
#pragma GCC diagnostic pop
  ...
  if (!err) {
    ...
    /*  make it possible to set measurement interval down to 1 second
 *  this is not inline with the Sensor.Community interval but may be necessary for other
 *  measurements
 *  
...
#else
  bool LittleFS_begin_ok = LittleFS.begin();
#endif

#pragma GCC diagnostic pop

  if (!LittleFS_begin_ok) {
...
static bool writeConfig() {
  ...
  if (cfg::send2dusti || cfg::send2madavi) {
    cfg::sending_intervall_ms = sc_sending_intervall_ms;
  }

  // only one microphone sjould be set
   if (cfg::dnms_ICS43434) {
    cfg::dnms_IM72D128 = false;
  }
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
  ...
  LittleFS.remove(F("/config.json.old"));
  LittleFS.rename(F("/config.json"), F("/config.json.old"));
...
//  loggerConfigs[LoggerMQTT].destport = cfg::port_mqtt;
...
static void webserver_root() {
  if (WiFi.status() != WL_CONNECTED) {
    ...
    RESERVE_STRING(page_content, XXLARGE_STR);
...
static void webserver_config_send_body_get(String &page_content) {
  ...
  page_content += FPSTR(INTL_DNMS_SENSOR);
  page_content += F("</label>"
...
*/
  ...
  page_content += FPSTR(INTL_NTP_SERVER_TEXT);
  page_content += FPSTR(WEB_BR);
  page_content += FPSTR(TABLE_TAG_OPEN);
  add_form_input(page_content, Config_ntp_1, FPSTR(INTL_NTP_SERVER_1), LEN_NTP_1 - 1);
  add_form_input(page_content, Config_ntp_2, FPSTR(INTL_NTP_SERVER_2), LEN_NTP_2 - 1);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);

  page_content += FPSTR(WEB_BR_BR);
  page_content += FPSTR(INTL_START);
  page_content += FPSTR(WEB_BR);  
  add_form_checkbox_sensor(Config_start_on_full_minute, FPSTR(INTL_START_ON_FULL_MINUTE));
  add_form_checkbox_sensor(Config_start_on_full_hour, FPSTR(INTL_START_ON_FULL_HOUR));
  page_content += FPSTR(WEB_LF_B);
  page_content += FPSTR(INTL_START_ZWEI);
  page_content += FPSTR(WEB_B_BR_BR);

  add_form_checkbox_sensor(Config_sensor_cyclic_restart, FPSTR(INTL_SENSOR_CYCLIC_RESTART));
  page_content += FPSTR(TABLE_TAG_OPEN);
  add_form_input(page_content, Config_sensor_cyclic_restart_time, FPSTR(INTL_SENSOR_CYCLIC_RESTART_TIME), 5);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);

  page_content += FPSTR(WEB_BR_BR);
  page_content += FPSTR(INTL_CAREFUL_ATTENTION);
  page_content += FPSTR(TABLE_TAG_OPEN);
  add_form_input(page_content, Config_i2c_gpio_scl, FPSTR(INTL_I2C_GPIO_SCL), 5);
  add_form_input(page_content, Config_i2c_gpio_sda, FPSTR(INTL_I2C_GPIO_SDA), 5);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);
  server.sendContent(page_content);

  page_content = tmpl(FPSTR(WEB_DIV_PANEL), String(3));
  add_form_checkbox_sensor(Config_dnms_read, FPSTR(INTL_DNMS));
  add_form_checkbox_sensor(Config_dnms_read_Z, FPSTR(INTL_DNMS_Z));
  add_form_checkbox_sensor(Config_dnms_read_spectrum, FPSTR(INTL_DNMS_SPECTRUM));
  add_form_checkbox_sensor(Config_dnms_read_spectrum_Z, FPSTR(INTL_DNMS_SPECTRUM_Z));
  page_content += FPSTR(WEB_BR_BR);
  add_form_checkbox_sensor(Config_dnms_2nd_interval, FPSTR(INTL_DNMS_2nd_INTERVAL));
  add_form_checkbox_sensor(Config_dnms_2nd_interval_Z, FPSTR(INTL_DNMS_2nd_INTERVAL_Z));
  add_form_checkbox_sensor(Config_dnms_2nd_read_spectrum, FPSTR(INTL_DNMS_2nd_SPECTRUM));
  add_form_checkbox_sensor(Config_dnms_2nd_read_spectrum_Z, FPSTR(INTL_DNMS_2nd_SPECTRUM_Z));
  page_content += FPSTR(WEB_BR_BR);
  // Paginate page after ~ 1500 Bytes
  server.sendContent(page_content);
  page_content = emptyString;
  ...
  page_content += FPSTR(TABLE_TAG_OPEN);
  add_form_input(page_content, Config_dnms_2nd_interval_time, FPSTR(INTL_DNMS_2nd_INTERVAL_TIME), 5);
  add_form_input(page_content, Config_dnms_2nd_threshold, FPSTR(INTL_DNMS_2nd_THRESHOLD), 5);
  add_form_input(page_content, Config_dnms_2nd_number_measurements, FPSTR(INTL_DNMS_2nd_NUMBER_MEASUREMENTS), 5);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);
  add_form_checkbox_sensor(Config_dnms_2nd_switch_output, FPSTR(INTL_DNMS_2nd_SWITCH_OUTPUT));
  page_content += FPSTR(TABLE_TAG_OPEN);
  add_form_input(page_content, Config_dnms_2nd_gpio_pin, FPSTR(INTL_DNMS_2nd_GPIO_PIN), 5);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);
  page_content += FPSTR(WEB_BR_BR);
  page_content += FPSTR(TABLE_TAG_OPEN);
  ...
  page_content += FPSTR(WEB_BR_BR);
  page_content += FPSTR(INTL_DNMS_MICROPHONE);
  page_content += FPSTR(WEB_BR); 
  add_form_checkbox_sensor(Config_dnms_ICS43434, FPSTR(INTL_DNMS_ICS43434));
  add_form_checkbox_sensor(Config_dnms_IM72D128, FPSTR(INTL_DNMS_IM72D128));
  ...
  add_form_checkbox_sensor(Config_scd4x_read, FPSTR(INTL_SCD4X));
  add_form_checkbox_sensor(Config_scd4x_low_power, FPSTR(INTL_SCD4X_LOW_POWER));
...
//  page_content += FPSTR(WEB_BRACE_BR);
  ...
  server.sendContent(page_content);
  page_content = form_checkbox(Config_send2influx, tmpl(FPSTR(INTL_SEND_TO), F("InfluxDB (V1.8)")), false);
//  page_content += FPSTR(WEB_NBSP_NBSP_BRACE);
...
//  page_content += FPSTR(WEB_BRACE_BR);
  ...
  add_form_input(page_content, Config_measurement_name_influx, FPSTR(INTL_INFLUX_MEASUREMENT), LEN_MEASUREMENT_NAME_INFLUX - 1);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);

  page_content += FPSTR(BR_TAG);

  server.sendContent(page_content);
  page_content = form_checkbox(Config_send2mqtt, tmpl(FPSTR(INTL_SEND_TO), F("MQTT Broker")), false);
//  page_content += FPSTR(WEB_NBSP_NBSP_BRACE);
//  page_content += form_checkbox(Config_tls_mqtt, FPSTR(WEB_TLS), false);
//  page_content += FPSTR(WEB_BRACE_BR);
  page_content += FPSTR(TABLE_TAG_OPEN);
  ...
  add_form_input(page_content, Config_measurement_name_mqtt, FPSTR(INTL_MQTT_MEASUREMENT), LEN_MEASUREMENT_NAME_MQTT - 1);
  add_form_input(page_content, Config_mqtt_main_topic, FPSTR(INTL_MQTT_MAIN_TOPIC), LEN_MQTT_MAIN_TOPIC - 1);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);
  add_form_checkbox_sensor(Config_id_as_mqtt_sub_topic, FPSTR(INTL_ID_AS_MQTT_SUB_TOPIC));
  page_content += FPSTR(TABLE_TAG_OPEN);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);
...
static void webserver_values() {
  ...
  if (cfg::sen5x_read) {
    add_table_pm_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_PM1), last_value_SEN5X_P0);
    add_table_pm_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_PM25), last_value_SEN5X_P2);
    add_table_pm_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_PM4), last_value_SEN5X_P4);
    add_table_pm_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_PM10), last_value_SEN5X_P1);
    add_table_nc_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_NC0k5), last_value_SEN5X_N05);
    add_table_nc_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_NC1k0), last_value_SEN5X_N1);
    add_table_nc_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_NC2k5), last_value_SEN5X_N25);
    add_table_nc_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_NC4k0), last_value_SEN5X_N4);
    add_table_nc_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_NC10), last_value_SEN5X_N10);
    add_table_value(FPSTR(SENSORS_SEN5X), FPSTR(WEB_TPS), check_display_value(last_value_SEN5X_TS, -1, 1, 0), "µm");
    if ((SEN5X_type[4] == '4') || (SEN5X_type[4] == '5')) {
      add_table_t_value(FPSTR(SENSORS_SEN5X), FPSTR(INTL_TEMPERATURE), last_value_SEN5X_T);
      add_table_h_value(FPSTR(SENSORS_SEN5X), FPSTR(INTL_HUMIDITY), last_value_SEN5X_H);
      add_table_value(FPSTR(SENSORS_SEN5X), FPSTR(INTL_VOC), check_display_value(last_value_SEN5X_VOC, 0, 0, 0), "(index)");
    }
    if (SEN5X_type[4] == '5') {
      add_table_value(FPSTR(SENSORS_SEN5X), FPSTR(INTL_NOX), check_display_value(last_value_SEN5X_NOX, 0, 0, 0), "(index)");
    }
    page_content += FPSTR(EMPTY_ROW);
  }
  if (cfg::dht_read) {
  ...
  if (cfg::scd4x_read) {
    add_table_t_value(FPSTR(SENSORS_SCD4X), FPSTR(INTL_TEMPERATURE), last_value_SCD4X_T);
    add_table_h_value(FPSTR(SENSORS_SCD4X), FPSTR(INTL_HUMIDITY), last_value_SCD4X_H);
    add_table_value(FPSTR(SENSORS_SCD4X), FPSTR(INTL_CO2_PPM), check_display_value(last_value_SCD4X_CO2, 0, 0, 0), unit_CO2);
    dew_point_temp = dew_point(last_value_SCD4X_T, last_value_SCD4X_H);
    add_table_value(FPSTR(SENSORS_SCD4X), FPSTR(INTL_DEW_POINT), isnan(dew_point_temp) ? "-" : String(dew_point_temp, 1), unit_T);
    page_content += FPSTR(EMPTY_ROW);
  }
  if (cfg::ds18b20_read) {
  ...
  if (cfg::dnms_read_Z_active) {
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LEQ_Z), check_display_value(last_value_dnms_lzeq, -1, 1, 0), unit_LZ);
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LZ_MIN), check_display_value(last_value_dnms_lz_min, -1, 1, 0), unit_LZ);
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LZ_MAX), check_display_value(last_value_dnms_lz_max, -1, 1, 0), unit_LZ);
    page_content += FPSTR(EMPTY_ROW);
  }
  if (cfg::dnms_read_spectrum_active) {
    for (unsigned i = 0; i < 31; i++) {
      add_table_value(FPSTR(SENSORS_DNMS), FPSTR(WEB_LAeq_HZ_2[i]), check_display_value(last_value_dnms_laeq_freq_spec[i], -1, 1, 0), unit_LA);
    }
    page_content += FPSTR(EMPTY_ROW);
  }
  if (cfg::dnms_read_spectrum_Z_active) {
    for (unsigned i = 0; i < 31; i++) {
      add_table_value(FPSTR(SENSORS_DNMS), FPSTR(WEB_LZeq_HZ_2[i]), check_display_value(last_value_dnms_lzeq_freq_spec[i], -1, 1, 0), unit_LZ);
    }
    page_content += FPSTR(EMPTY_ROW);
  }
  if (cfg::dnms_2nd_interval_active) {
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LEQ_A_2nd), check_display_value(last_value_dnms_laeq_2nd, -1, 1, 0), unit_LA);
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LA_MIN_2nd), check_display_value(last_value_dnms_la_min_2nd, -1, 1, 0), unit_LA);
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LA_MAX_2nd), check_display_value(last_value_dnms_la_max_2nd, -1, 1, 0), unit_LA);
    page_content += FPSTR(EMPTY_ROW);
  }
  if (cfg::dnms_2nd_interval_Z_active) {
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LEQ_Z_2nd), check_display_value(last_value_dnms_lzeq_2nd, -1, 1, 0), unit_LZ);
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LZ_MIN_2nd), check_display_value(last_value_dnms_lz_min_2nd, -1, 1, 0), unit_LZ);
    add_table_value(FPSTR(SENSORS_DNMS), FPSTR(INTL_LZ_MAX_2nd), check_display_value(last_value_dnms_lz_max_2nd, -1, 1, 0), unit_LZ);
    page_content += FPSTR(EMPTY_ROW);
  }
  if (cfg::dnms_2nd_read_spectrum_active) {
    for (unsigned i = 0; i < 31; i++) {
      add_table_value(FPSTR(SENSORS_DNMS), FPSTR(WEB_LAeq_HZ_2_2nd[i]), check_display_value(last_value_dnms_laeq_freq_spec_2nd[i], -1, 1, 0), unit_LA);
    }
    page_content += FPSTR(EMPTY_ROW);
  }
  if (cfg::dnms_2nd_read_spectrum_Z_active) {
    for (unsigned i = 0; i < 31; i++) {
      add_table_value(FPSTR(SENSORS_DNMS), FPSTR(WEB_LZeq_HZ_2_2nd[i]), check_display_value(last_value_dnms_lzeq_freq_spec_2nd[i], -1, 1, 0), unit_LZ);
    }
    page_content += FPSTR(EMPTY_ROW);
  }
...
static void webserver_status() {
  ...
  if (cfg::scd30_read) {
    if (scd30_init_failed) {
      add_table_row_from_value(page_content, FPSTR(SENSORS_SCD30), String("init failed"));
    } else {
      add_table_row_from_value(page_content, F("SCD30 firmware"), String(last_value_SCD30_firmware));
      if (scd30.getAutoSelfCalibration() == true) {
        add_table_row_from_value(page_content, F("SCD30 auto calibration"), String("enabled"));
      } else {
        add_table_row_from_value(page_content, F("SCD30 auto calibration"), String("disabled"));
      }
      add_table_row_from_value(page_content, F("SCD30 measurement interval"), String(scd30.getMeasurementInterval()));
      add_table_row_from_value(page_content, F("SCD30 sensor altitude"), String(scd30.getAltitudeCompensation()));
      add_table_row_from_value(page_content, F("SCD30 temperature offset"), String(scd30.getTemperatureOffset()));
    }
  }
  if (cfg::scd4x_read) {
    if (scd4x_init_failed) {
      add_table_row_from_value(page_content, FPSTR(SENSORS_SCD4X), String("init failed"));
    } else {
      add_table_row_from_value(page_content, F("SCD4X serial number"), last_value_SCD4X_serialNumber);
      scd4x.stopPeriodicMeasurement();
      if (scd4x.getAutomaticSelfCalibrationEnabled()) {
        add_table_row_from_value(page_content, F("SCD4X Auto Calibration"), String("enabled"));
      } else {
        add_table_row_from_value(page_content, F("SCD4X Auto Calibration"), String("disabled"));
      }
      add_table_row_from_value(page_content, F("SCD4X temperature offset"), String(scd4x.getTemperatureOffset()));
      add_table_row_from_value(page_content, F("SCD4X sensor altitude"), String(scd4x.getSensorAltitude()));
      if (cfg::scd4x_low_power) {
        add_table_row_from_value(page_content, F("SCD4X low power"), String("enabled"));
        scd4x.startLowPowerPeriodicMeasurement();
      } else {
        add_table_row_from_value(page_content, F("SCD4X low power"), String("disabled"));
  ...
  if (cfg::dnms_read) {
    if (dnms_init_failed) {
      add_table_row_from_value(page_content, FPSTR(SENSORS_DNMS), String("init failed"));
    } else {
      add_table_row_from_value(page_content, FPSTR(SENSORS_DNMS), last_value_DNMS_version);
      add_table_row_from_value(page_content, FPSTR(INTL_DNMS_CORRECTION), String(readCorrectionOffset(cfg::dnms_correction)));
    }
  }
  if (cfg::sen5x_read) {
    if (sen5x_init_failed) {
      add_table_row_from_value(page_content, FPSTR(SENSORS_SEN5X), String("init failed"));
    } else {
      add_table_row_from_value(page_content, FPSTR(SENSORS_SEN5X), last_value_SEN5X_serialNumber);
      add_table_row_from_value(page_content, F("SEN5X Type"), String((char *)SEN5X_type));
      add_table_row_from_value(page_content, F("SEN5X SW"), last_value_SEN5X_sw);
      add_table_row_from_value(page_content, F("SEN5X HW"), last_value_SEN5X_hw);
      sen5x.getTemperatureOffsetSimple(sen5x_temp);
      add_table_row_from_value(page_content, F("SEN5X temperature offset"), String(sen5x_temp));
    }
  }
  if (cfg::sps30_read) {
    if (sps30_init_failed) {
      add_table_row_from_value(page_content, FPSTR(SENSORS_SPS30), String("init failed"));
    } else {
      add_table_row_from_value(page_content, FPSTR(SENSORS_SPS30), last_value_SPS30_version);
    }
  }
  if (cfg::sht3x_read) {
    if (sht3x_init_failed) {
      add_table_row_from_value(page_content, FPSTR(SENSORS_SHT3X), String("init failed"));
    } else {
      add_table_row_from_value(page_content, FPSTR(SENSORS_SHT3X), String("initialized"));
    }
    add_table_row_from_value(page_content, F("SHT3x temperature offset"), String(float(readCorrectionOffset(cfg::temp_correction))));
  }
  if (cfg::bmx280_read) {
    const char *const sensor_name = (bmx280.sensorID() == BME280_SENSOR_ID) ? SENSORS_BME280 : SENSORS_BMP280;
    debug_outln_verbose(FPSTR(DBG_TXT_START_READING), FPSTR(sensor_name));
    if (bmx280_init_failed) {
      add_table_row_from_value(page_content, FPSTR(sensor_name), String("init failed"));
    } else {
      add_table_row_from_value(page_content, FPSTR(sensor_name), String("initialized"));
    }
    add_table_row_from_value(page_content, F("BME280 temperature offset"), String(float(readCorrectionOffset(cfg::temp_correction))));
  }
  page_content += FPSTR(EMPTY_ROW);
  ...
  if (cfg::sen5x_read) {
    add_table_row_from_value(page_content, FPSTR(SENSORS_SEN5X), String(SEN5X_read_error_counter));
  }
  if (cfg::dnms_read) {
    add_table_row_from_value(page_content, FPSTR(SENSORS_DNMS), String(DNMS_error_count));
  }
  if (cfg::scd30_read) {
    add_table_row_from_value(page_content, FPSTR(SENSORS_SCD30), String(SCD30_read_error_counter));
  }
  if (cfg::scd4x_read) {
    add_table_row_from_value(page_content, FPSTR(SENSORS_SCD4X), String(SCD4X_read_error_counter));
  }
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    ...
    LittleFS.remove(F("/config.json.old"));
    if (LittleFS.exists(F("/config.json"))) {  //file exists
      debug_outln_info(F("removing config.json..."));
      if (LittleFS.remove(F("/config.json"))) {
...
static void webserver_metrics_endpoint() {
  ...
  RESERVE_STRING(page_content, XXXLARGE_STR);
...
static void webserver_not_found() {
  last_page_load = millis();
  debug_outln_error(F("ws: not found ..."));
  if (WiFi.status() != WL_CONNECTED) {
...
static void wifiConfig() {
  ...
  debug_outln_info_bool(F("DNMS READ SPECTRUM: "), cfg::dnms_read_spectrum);
  debug_outln_info_bool(F("DNMS 2nd INTERVAL: "), cfg::dnms_2nd_interval);
  debug_outln_info(FPSTR(DBG_TXT_SEP));
...
static void connectWifi() {
  ...
  if (addr_static_ip.fromString(cfg::static_ip) && addr_static_subnet.fromString(cfg::static_subnet) && addr_static_gateway.fromString(cfg::static_gateway) && addr_static_dns.fromString(cfg::static_dns)) {
    if (!WiFi.config(addr_static_ip, addr_static_gateway, addr_static_subnet, addr_static_dns, addr_static_dns))
      Debug.println("setting of static IP failed!");
  }
#endif
...
static unsigned long sendData(const LoggerEntry logger, const String &data, const int pin, const char *host, const char *url) {
  ...
  if (cfg::send2mqtt && logger== LoggerMQTT) {
    if (!mqtt_client.connected()) {   // not connected
      // Attempt to connect
      if (mqtt_client.connect(esp_chipid.c_str(),cfg::user_mqtt, cfg::pwd_mqtt)) {
      Debug.println("MQTT Broker connected");
      } else {
        for (int i=0; i < 10; i++) {
          Debug.print("failed to connect to MQTT Broker, rc=");
          Debug.println(mqtt_client.state());
          Debug.println(" try again in 100 ms");
          delay(100);
        }
      }
    }
    if (mqtt_client.connected()) {  // by now MQTT Broker should be connected
      if (!mqtt_client.publish(mqtt_topic.c_str(), data.c_str())) {
        // error handling?
      }
    } else {  // not connected error!!
      Debug.print("no connction to MQTT Broker, rc=");
      Debug.println(mqtt_client.state());
    }
...
/*
      if (logger == LoggerSensorCommunity) {
        Debug.println(data);
      }
*/
      ...
      if (result >= HTTP_CODE_OK && result <= 299) {
        debug_outln_info(F("Succeeded - "), s_Host);
        send_success = true;
      } else if (result >= HTTP_CODE_BAD_REQUEST || result < 0) {

        Debug.print("error result: "); Debug.println(result);

        time_t now = time(nullptr);
        debug_out(FPSTR(INTL_TIME_UTC), DEBUG_ERROR);
        Debug.print(String(ctime(&now)) + '\r');
        debug_out("http POST to destination: ", DEBUG_ERROR);
        switch (logger) {
          case 0: debug_outln("Sensor.Community", DEBUG_ERROR); break;
          case 1: debug_outln("Madavi", DEBUG_ERROR); break;
          case 2: debug_outln("OpenSenseMap", DEBUG_ERROR); break;
          case 3: debug_outln("PM App", DEBUG_ERROR); break;
          case 4: debug_outln("aircms", DEBUG_ERROR); break;
          case 5: debug_outln("InfluxDB", DEBUG_ERROR); break;
          case 6: debug_outln("Own API", DEBUG_ERROR); break;
          default:
            debug_outln("not specified: ", DEBUG_ERROR);
            debug_outln_error(String(logger));
            break;
        }
        debug_out(F("Request failed with HTTP error: "), DEBUG_ERROR);
        //      debug_outln_error(String(result));
        switch (result) {
          case -1: debug_outln("connection failed", DEBUG_ERROR); 
            for (int ii=0; ii < 3; ii++) {
              delay(20);
              result = http.POST(data);
              Debug.print("retransmission result: "); Debug.println(result);
              if(result >= HTTP_CODE_OK && result <= 299) {
                break;
              }
            }        
          break;
          case -2: debug_outln("send header failed", DEBUG_ERROR); break;
          case -3: debug_outln("send payload failed", DEBUG_ERROR); 
            for (int ii=0; ii < 3; ii++) {
              delay(20);
              result = http.POST(data);
              Debug.print("retransmission result: "); Debug.println(result);
            ...
            break;
          case -4: debug_outln("not connected", DEBUG_ERROR); break;
          case -5: debug_outln("connection lost", DEBUG_ERROR); break;
          case -6: debug_outln("no stream", DEBUG_ERROR); break;
          case -7: debug_outln("no HTTP server", DEBUG_ERROR); break;
          case -8: debug_outln("too less RAM", DEBUG_ERROR); break;
          case -9: debug_outln("encoding", DEBUG_ERROR); break;
          case -10: debug_outln("stream write", DEBUG_ERROR); break;
          case -11: debug_outln("read timeout", DEBUG_ERROR); break;
          default: debug_outln_error(String(result)); break;
        }
        debug_out(F("Details:"), DEBUG_ERROR);
        debug_outln_error(http.getString());
        debug_outln("----", DEBUG_ERROR);
      }
      http.end();
    } else {
      debug_outln_error(F("Failed connecting to: "));
      debug_outln_error(String(s_Host));
    }
 ...
 *****************************************************************
  ...
  where_to = 1 measurement name for InfluxDB is inserted
  where_to = 2 measurement name for MQTT is inerted
 
 */
static void create_influxdb_string_from_data(String &data_4_influxdb, const String &data, const String &measure_name) {
  debug_outln_verbose(F("Parse JSON for influx DB: "), data);
  DynamicJsonDocument json2data(JSON_BUFFER_SIZE);
  DeserializationError err = deserializeJson(json2data, data);
  if (!err) {
    data_4_influxdb += measure_name;    
    ...
    // input time from DNMS measurement in data string for transmission to influxdb
    data_4_influxdb += " " + String(dnms_now) + "000000000";

    data_4_influxdb += '\n';

    //    Debug.println(data_4_influxdb);

  } else {
    debug_out(FPSTR(INTL_TIME_UTC), DEBUG_ERROR);
    Debug.print(String(ctime(&dnms_now)) + '\r');
    debug_out("error creating influxDB string from data: ", DEBUG_ERROR);
...
static void fetchSensorDHT(String &s) {
  ...
  while ((count++ < MAX_ATTEMPTS)) {
    ...
    if (isnan(t) || isnan(h)) {
      ...
      last_value_DHT_T = t + temp_corr_value;
      last_value_DHT_H = h;
...
static void fetchSensorSHT3x(String &s) {
  ...
  if (isnan(h) || isnan(t)) {
    ...
    last_value_SHT3X_T = t + temp_corr_value;
...
/*****************************************************************
 * read SCD4X sensor values                                      *
 *****************************************************************/
static void fetchSensorSCD4X(String &s) {
  debug_outln_verbose(FPSTR(DBG_TXT_START_READING), FPSTR(SENSORS_SCD4X));

  const auto t = scd4x.getTemperature();
  const auto h = scd4x.getHumidity();
  const auto c = scd4x.getCO2();

  if (isnan(h) || isnan(t) || isnan(c)) {
    last_value_SCD4X_T = -128.0;
    last_value_SCD4X_H = -1.0;
    last_value_SCD4X_CO2 = 0;
    debug_outln_error(F("SCD4X read failed"));
    ++SCD4X_read_error_counter;
  } else {
    last_value_SCD4X_T = t;
    last_value_SCD4X_H = h;
    last_value_SCD4X_CO2 = c;
    add_Value2Json(s, F("SCD4X_temperature"), FPSTR(DBG_TXT_TEMPERATURE), last_value_SCD4X_T);
    add_Value2Json(s, F("SCD4X_humidity"), FPSTR(DBG_TXT_HUMIDITY), last_value_SCD4X_H);
    add_Value2Json(s, F("SCD4X_co2_ppm"), FPSTR(DBG_TXT_CO2PPM), last_value_SCD4X_CO2);
  }
  debug_outln_info(FPSTR(DBG_TXT_SEP));
  debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_SCD4X));
}
...
static void fetchSensorBMX280(String &s) {
  ...
  if (isnan(t) || isnan(p)) {
    ...
    last_value_BMX280_T = t + temp_corr_value;
...
static void fetchSensorDS18B20(String &s) {
  ...
  if (count == MAX_ATTEMPTS) {
    ...
    last_value_DS18B20_T = t + temp_corr_value;
...
static void fetchSensorSDS(String &s) {
  if (cfg::sending_intervall_ms > (WARMUPTIME_SDS_MS + READINGTIME_SDS_MS) && msSince(starttime) < (cfg::sending_intervall_ms - (WARMUPTIME_SDS_MS + READINGTIME_SDS_MS))) {
    if (is_SDS_running) {
...
static void fetchSensorIPS(String &s) {
  ...
  if (cfg::sending_intervall_ms > (WARMUPTIME_IPS_MS + READINGTIME_IPS_MS) && msSince(starttime) < (cfg::sending_intervall_ms - (WARMUPTIME_IPS_MS + READINGTIME_IPS_MS))) {
    ...
    if (msSince(starttime) > (cfg::sending_intervall_ms - READINGTIME_IPS_MS)) {  //DIMINUER LE READING TIME
...
static void fetchSensorSEN5X(String &s) {
  ...
  add_Value2Json(s, F("SEN5X_P2"), F("PM2.5: "), last_value_SEN5X_P2);
  add_Value2Json(s, F("SEN5X_P4"), F("PM4.0: "), last_value_SEN5X_P4);
  add_Value2Json(s, F("SEN5X_P1"), F("PM 10: "), last_value_SEN5X_P1);
  add_Value2Json(s, F("SEN5X_N05"), F("NC0.5: "), last_value_SEN5X_N05);
  add_Value2Json(s, F("SEN5X_N1"), F("NC1.0: "), last_value_SEN5X_N1);
  add_Value2Json(s, F("SEN5X_N25"), F("NC2.5: "), last_value_SEN5X_N25);
  add_Value2Json(s, F("SEN5X_N4"), F("NC4.0: "), last_value_SEN5X_N4);
...
static void fetchSensorDNMS(String &s) {
  ...
  if (!dnms_error) {
    ...
    if (dnms_read_leq(DNMS_CMD_READ_LEQ, &dnms_values) == 0) {
      last_value_dnms_laeq = dnms_values.leq_x + dnms_corr_value;
      last_value_dnms_la_min = dnms_values.leq_x_min + dnms_corr_value;
      last_value_dnms_la_max = dnms_values.leq_x_max + dnms_corr_value;
      dnms_now = time(nullptr);
    } else {
      dnms_error = true;
    }
  }
  if (dnms_error) {
    DNMS_error_count++;
    DNMS_error_continous_count++;
    debug_outln_error(F("DNMS LAeq read failed"));
    dnms_reset();  // try to reset dnms
    if (DNMS_error_continous_count == DNMS_ERROR_COUNT_MAX) {
      sensor_restart();
    }

  } else {
    DNMS_error_continous_count = 0;
...
static void fetchSensorDNMS_Z(String &s) {
  dnms_error = false;
  struct dnms_measurements dnms_values;
  if (dnms_read_leq(DNMS_CMD_READ_LEQ_Z, &dnms_values) == 0) {
    last_value_dnms_lzeq = dnms_values.leq_x + dnms_corr_value;
    last_value_dnms_lz_min = dnms_values.leq_x_min + dnms_corr_value;
    last_value_dnms_lz_max = dnms_values.leq_x_max + dnms_corr_value;
  } else {
    dnms_error = true;
  }
  if (dnms_error) {
    DNMS_error_count++;
    DNMS_error_continous_count++;
    debug_outln_error(F("DNMS LZeq read failed"));
    dnms_reset();  // try to reset dnms
    if (DNMS_error_continous_count == DNMS_ERROR_COUNT_MAX) {
      sensor_restart();
    }
    ...
    DNMS_error_continous_count = 0;
    add_Value2Json(s, F("DNMS_noise_LZeq"), F("LZeq: "), last_value_dnms_lzeq);
    add_Value2Json(s, F("DNMS_noise_LZ_min"), F("LZ_MIN: "), last_value_dnms_lz_min);
    add_Value2Json(s, F("DNMS_noise_LZ_max"), F("LZ_MAX: "), last_value_dnms_lz_max);
  }
  debug_outln_info(FPSTR(DBG_TXT_SEP));
  debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_DNMS));
}
...
static void fetchSensorDNMS_FFT(String &s) {
  dnms_error = false;
  struct dnms_measurements dnms_values;
  debug_outln_verbose(FPSTR(DBG_TXT_START_READING), FPSTR(SENSORS_DNMS));
  for (unsigned i = 0; i < 31; i++) {
    last_value_dnms_laeq_freq_spec[i] = -1.0;
  }
  // ***** read dnms frequency spectrum if dnms firmware version = 3 and if configured
  if (dnms_read_freq_spec(DNMS_CMD_READ_FFT_PART1, &dnms_values) == 0) {
    for (unsigned i = 0; i < 31; i++) {
      last_value_dnms_laeq_freq_spec[i] = dnms_values.leq_freq_spec_x[i];
    }
  } else {
    dnms_error = true;
  }
  if (dnms_error) {
    DNMS_error_count++;
    DNMS_error_continous_count++;
    debug_outln_error(F("DNMS LA spectrum read failed"));
    dnms_reset();  // try to reset dnms
    if (DNMS_error_continous_count == DNMS_ERROR_COUNT_MAX) {
      sensor_restart();
    }
    ...
    for (unsigned i = 0; i < 31; i++) {
      add_Value2Json(s, FPSTR(WEB_LAeq_HZ[i]), FPSTR(WEB_LAeq_HZ_2[i]), last_value_dnms_laeq_freq_spec[i]);
    }
  }
  debug_outln_info(FPSTR(DBG_TXT_SEP));
  debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_DNMS));
...
static void fetchSensorDNMS_FFT_Z(String &s) {
  dnms_error = false;
  struct dnms_measurements dnms_values;
  debug_outln_verbose(FPSTR(DBG_TXT_START_READING), FPSTR(SENSORS_DNMS));
  for (unsigned i = 0; i < 31; i++) {
    last_value_dnms_lzeq_freq_spec[i] = -1.0;
  }
  // ***** read dnms frequency spectrum if dnms firmware version = 3 and if configured
  if (dnms_read_freq_spec(DNMS_CMD_READ_FFT_Z_PART1, &dnms_values) == 0) {
    for (unsigned i = 0; i < 31; i++) {
      last_value_dnms_lzeq_freq_spec[i] = dnms_values.leq_freq_spec_x[i];
    }
  } else {
    dnms_error = true;
  }
  if (dnms_error) {
    DNMS_error_count++;
    DNMS_error_continous_count++;
    debug_outln_error(F("DNMS LZ spectrum read failed"));
    dnms_reset();  // try to reset dnms
    if (DNMS_error_continous_count == DNMS_ERROR_COUNT_MAX) {
      sensor_restart();
    }
    ...
    for (unsigned i = 0; i < 31; i++) {
      add_Value2Json(s, FPSTR(WEB_LZeq_HZ[i]), FPSTR(WEB_LZeq_HZ_2[i]), last_value_dnms_lzeq_freq_spec[i]);
    }
  }
  debug_outln_info(FPSTR(DBG_TXT_SEP));
  debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_DNMS));
...
static void fetchSensorDNMS_2nd(String &s) {
  dnms_error = false;
  debug_outln_verbose(FPSTR(DBG_TXT_START_READING), FPSTR(SENSORS_DNMS));
  last_value_dnms_laeq_2nd = -1.0;
  last_value_dnms_la_min_2nd = -1.0;
  last_value_dnms_la_max_2nd = -1.0;
  ...
  for (unsigned i = 0; i < 20; i++) {
    delay(2);
    int16_t ret_dnms = dnms_read_data_ready_2nd(&data_ready);
    if ((ret_dnms == 0) && (data_ready != 0)) {
      dnms_error = false;
      break;
    }
  ...
  if (!dnms_error) {
    ...
    if (dnms_read_leq(DNMS_CMD_READ_LEQ_2nd, &dnms_values) == 0) {
      last_value_dnms_laeq_2nd = dnms_values.leq_x + dnms_corr_value;
      last_value_dnms_la_min_2nd = dnms_values.leq_x_min + dnms_corr_value;
      last_value_dnms_la_max_2nd = dnms_values.leq_x_max + dnms_corr_value;
      dnms_2nd_now = time(nullptr);
    } else {
      dnms_error = true;
    }
  }
  if (dnms_error) {
    DNMS_error_count++;
    DNMS_error_continous_count++;
    debug_outln_error(F("DNMS LAeq 2nd read failed"));
    dnms_reset();  // try to reset dnms
    if (DNMS_error_continous_count == DNMS_ERROR_COUNT_MAX) {
      sensor_restart();
    }
    ...
    DNMS_error_continous_count = 0;
    if ((last_value_dnms_laeq_2nd >= cfg::dnms_2nd_threshold) || (dnms_2nd_count > 0)) {
      add_Value2Json(s, F("DNMS_noise_LAeq_2nd"), F("LAeq_2nd: "), last_value_dnms_laeq_2nd);
      add_Value2Json(s, F("DNMS_noise_LA_min_2nd"), F("LA_MIN_2nd: "), last_value_dnms_la_min_2nd);
      add_Value2Json(s, F("DNMS_noise_LA_max_2nd"), F("LA_MAX_2nd: "), last_value_dnms_la_max_2nd);
      debug_outln_info(FPSTR(DBG_TXT_SEP));
      debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_DNMS));
    } else {
      add_Value2Json2(s, F("DNMS_noise_LAeq_2nd"), F("LAeq_2nd: "), last_value_dnms_laeq_2nd);
      add_Value2Json2(s, F("DNMS_noise_LA_min_2nd"), F("LA_MIN_2nd: "), last_value_dnms_la_min_2nd);
      add_Value2Json2(s, F("DNMS_noise_LA_max_2nd"), F("LA_MAX_2nd: "), last_value_dnms_la_max_2nd);
    }
  }
}

static void fetchSensorDNMS_Z_2nd(String &s) {
  dnms_error = false;
  struct dnms_measurements dnms_values;
  if (dnms_read_leq(DNMS_CMD_READ_LEQ_Z_2nd, &dnms_values) == 0) {
    last_value_dnms_lzeq_2nd = dnms_values.leq_x + dnms_corr_value;
    last_value_dnms_lz_min_2nd = dnms_values.leq_x_min + dnms_corr_value;
    last_value_dnms_lz_max_2nd = dnms_values.leq_x_max + dnms_corr_value;
  } else {
    dnms_error = true;
  }
  if (dnms_error) {
    DNMS_error_count++;
    DNMS_error_continous_count++;
    debug_outln_error(F("DNMS LZeq 2nd read failed"));
    dnms_reset();  // try to reset dnms
    if (DNMS_error_continous_count == DNMS_ERROR_COUNT_MAX) {
      sensor_restart();
    }
    ...
    DNMS_error_continous_count = 0;
    add_Value2Json(s, F("DNMS_noise_LZeq_2nd"), F("LZeq_2nd: "), last_value_dnms_lzeq_2nd);
    add_Value2Json(s, F("DNMS_noise_LZ_min_2nd"), F("LZ_MIN_2nd: "), last_value_dnms_lz_min_2nd);
    add_Value2Json(s, F("DNMS_noise_LZ_max_2nd"), F("LZ_MAX_2nd: "), last_value_dnms_lz_max_2nd);
  }
  debug_outln_info(FPSTR(DBG_TXT_SEP));
  debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_DNMS));
}

static void fetchSensorDNMS_FFT_2nd(String &s) {
  static bool dnms_error = false;
  struct dnms_measurements dnms_values;
  debug_outln_verbose(FPSTR(DBG_TXT_START_READING), FPSTR(SENSORS_DNMS));
  for (unsigned i = 0; i < 31; i++) {
    last_value_dnms_laeq_freq_spec_2nd[i] = -1.0;
  }
  // ***** read dnms frequency spectrum if dnms firmware version = 3 and if configured
  if (dnms_read_freq_spec(DNMS_CMD_READ_FFT_PART1_2nd, &dnms_values) == 0) {
    for (unsigned i = 0; i < 31; i++) {
      last_value_dnms_laeq_freq_spec_2nd[i] = dnms_values.leq_freq_spec_x[i];
    }
  } else {
    dnms_error = true;
  }
  if (dnms_error) {
    DNMS_error_count++;
    DNMS_error_continous_count++;
    debug_outln_error(F("DNMS LA spectrum 2nd read failed"));
    dnms_reset();  // try to reset dnms
    if (DNMS_error_continous_count == DNMS_ERROR_COUNT_MAX) {
      sensor_restart();
    }
    ...
    // ***** read dnms frequency spectrum if dnms firmware version = 3 and if configured
    for (unsigned i = 0; i < 31; i++) {
      add_Value2Json(s, FPSTR(WEB_LAeq_HZ_2nd[i]), FPSTR(WEB_LAeq_HZ_2_2nd[i]), last_value_dnms_laeq_freq_spec_2nd[i]);
    }
  }
  debug_outln_info(FPSTR(DBG_TXT_SEP));
  debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_DNMS));
...
static void fetchSensorDNMS_FFT_Z_2nd(String &s) {
  dnms_error = false;
  struct dnms_measurements dnms_values;
  debug_outln_verbose(FPSTR(DBG_TXT_START_READING), FPSTR(SENSORS_DNMS));
  for (unsigned i = 0; i < 31; i++) {
    last_value_dnms_lzeq_freq_spec_2nd[i] = -1.0;
  }
  // ***** read dnms frequency spectrum if dnms firmware version = 3 and if configured
  if (dnms_read_freq_spec(DNMS_CMD_READ_FFT_Z_PART1_2nd, &dnms_values) == 0) {
    for (unsigned i = 0; i < 31; i++) {
      last_value_dnms_lzeq_freq_spec_2nd[i] = dnms_values.leq_freq_spec_x[i];
    }
  } else {
    dnms_error = true;
  }
  if (dnms_error) {
    DNMS_error_count++;
    DNMS_error_continous_count++;
    debug_outln_error(F("DNMS LZ spectrum 2nd read failed"));
    dnms_reset();  // try to reset dnms
    if (DNMS_error_continous_count == DNMS_ERROR_COUNT_MAX) {
      sensor_restart();
    }
    ...
    for (unsigned i = 0; i < 31; i++) {
      add_Value2Json(s, FPSTR(WEB_LZeq_HZ_2nd[i]), FPSTR(WEB_LZeq_HZ_2_2nd[i]), last_value_dnms_lzeq_freq_spec_2nd[i]);
    }
  }
  debug_outln_info(FPSTR(DBG_TXT_SEP));
  debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_DNMS));
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
  File fwFile = LittleFS.open(fname_new, "w");
  if (fwFile) {
    downloadSuccess = fwDownloadStream(client, url, &fwFile);
    fwFile.close();
    if (downloadSuccess) {
      LittleFS.remove(fname);
      LittleFS.rename(fname_new, fname);
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
  File fwFile = LittleFS.open(firmware_name, "r");
  if (!fwFile) {
    LittleFS.remove(firmware_name);
    LittleFS.remove(firmware_md5);
...
static void display_values() {
  ...
  if (cfg::sen5x_read) {
    ...
    pm01_sensor = pm10_sensor = pm25_sensor = pm05_sensor = FPSTR(SENSORS_SEN5X);
    if ((SEN5X_type[4] == '4') || (SEN5X_type[4] == '5')) {
      t_sensor = h_sensor = voc_sensor = FPSTR(SENSORS_SEN5X);
      t_value = last_value_SEN5X_T;
      h_value = last_value_SEN5X_H;
      voc_value = last_value_SEN5X_VOC;
    }
    if (SEN5X_type[4] == '5') {
      nox_sensor = FPSTR(SENSORS_SEN5X);
  ...
  if (cfg::scd30_read || cfg::scd4x_read) {
    ...
    switch (screens[next_display_count % screen_count]) {
      ...
      case 4:
        if (cfg::scd30_read) {
          ...
          display_header = "SCD4X";
          display_lines[0] = "Temp.: ";
          display_lines[0] += check_display_value(last_value_SCD4X_T, -128, 1, 5);
          display_lines[0] += " °C";
          display_lines[1] = "Hum.:  ";
          display_lines[1] += check_display_value(last_value_SCD4X_H, -1, 1, 5);
          display_lines[1] += " %";
          display_lines[2] = "CO2:   ";
          display_lines[2] += check_display_value(last_value_SCD4X_CO2, 0, 0, 5);
          display_lines[2] += " ppm";
        }
        break;
      case 5:
    ...
    switch (screens[next_display_count % screen_count]) {
      ...
      case 4:
        if (cfg::scd30_read) {
          ...
          display_lines[0] = std::move(tmpl(F("T/H: {v}"), check_display_value(last_value_SCD4X_T, -128, 1, 5) + " / " + check_display_value(last_value_SCD4X_H, -1, 0, 3)));
          display_lines[1] = std::move(tmpl(F("CO2: {v} ppm"), check_display_value(last_value_SCD4X_CO2, 0, 0, 6)));
        }
        break;
      case 5:
        display_lines[0] = "Lat: ";
...
static bool initBMX280(char addr) {
  ...
  if (bmx280.begin(addr)) {
    ...
    debug_outln_error(FPSTR(DBG_TXT_NOT_FOUND));
    return false;
  }
}
 ...
 * Init SHT3x                                                    *
 *****************************************************************/
static bool initSHT3x(char addr) {
  debug_out(String(F("Trying SHT3x sensor on ")) + String(addr, HEX), DEBUG_MIN_INFO);
  if (sht3x.begin(addr)) {
    debug_outln_info(FPSTR(DBG_TXT_FOUND));
    return true;
  } else {
...
static void initSCD30() {
  if (!scd30.begin()) {
    ...
    scd30.getFirmwareVersion(&last_value_SCD30_firmware);
    Debug.print("SCD30 firmware: ");
    Debug.println(last_value_SCD30_firmware);
    scd30.setAutoSelfCalibration(true);
    if (scd30.getAutoSelfCalibration()) {
      Debug.println("SCD30 auto self calibration: enabled");
    } else {
      Debug.println("SCD30 auto self calibration: disabled");
    }
    scd30.setMeasurementInterval(30);
    Debug.print("SCD30 measurement interval: ");
    Debug.println(scd30.getMeasurementInterval());
    scd30.setAltitudeCompensation(uint16_t(readCorrectionOffset(cfg::height_above_sealevel)));
    Debug.print("SCD30 sensor altitude: ");
    Debug.println(scd30.getAltitudeCompensation());
    scd30.setTemperatureOffset(float(readCorrectionOffset(cfg::temp_correction)));
    Debug.print("SCD30 temperature offset: ");
    Debug.println(scd30.getTemperatureOffset());
...
static void initSCD4X() {
  ...
  if (!scd4x.begin()) {
    debug_outln_error(F("Check SCD4X wiring"));
    scd4x_init_failed = true;
  } else {
    scd4x.stopPeriodicMeasurement();
    scd4x.getSerialNumber(serialNumber);
    Debug.print("SCD4X SerialNumber: ");
    Debug.println((char *)serialNumber);
    last_value_SCD4X_serialNumber = String((char *)serialNumber);
    scd4x.setTemperatureOffset(float(readCorrectionOffset(cfg::temp_correction)));
    Debug.print("SCD4X tmperature offset: ");
    Debug.println(scd4x.getTemperatureOffset());
    scd4x.setSensorAltitude(uint16_t(readCorrectionOffset(cfg::height_above_sealevel)));
    Debug.print("SCD4X height above sealevel: ");
    Debug.println(scd4x.getSensorAltitude());
    scd4x.setAutomaticSelfCalibrationEnabled(true, 5);
    if (scd4x.getAutomaticSelfCalibrationEnabled()) {
      Debug.println("SCD4X self calibration enabled");
    } else {
      Debug.println("SCD4X self calibration disabled");
    }
    if (cfg::scd4x_low_power) {
      ...
      if (scd4x.startPeriodicMeasurement() != true) {
        debug_outln_error(F("Check SCD4X wiring"));
        scd4x_init_failed = true;
      }
    }
...
static void initSEN5X() {
  debug_out(F("Trying SEN5X sensor"), DEBUG_MIN_INFO);
  ...
  sen5x.setTemperatureOffsetSimple(float(readCorrectionOffset(cfg::temp_correction)));
  sen5x.getTemperatureOffsetSimple(sen5x_temp);
  Debug.print("SEN5X tmperature offset: ");
  Debug.println(sen5x_temp);
...
static void initSPS30() {
  ...
  if (sps30_get_serial(serial) != 0) {
    ...
    debug_outln_error(F("Check SPS30 wiring"));
    sps30_init_failed = true;
    return;
  }
  debug_outln_info(F(" ... found, Serial-No.: "), String(serial));
  last_value_SPS30_version = String(serial);
...
static void initDNMS() {
  ...
  if (dnms_read_version(dnms_version) != 0) {
    ...
    dnms_corr_value = readCorrectionOffset(cfg::dnms_correction);
    dnms_version[DNMS_MAX_VERSION_LEN] = 0;
    last_value_DNMS_version = String(dnms_version);
    debug_outln_info(FPSTR(DBG_TXT_FOUND), String(": ") + last_value_DNMS_version);
    cfg::dnms_read_active = false;
    cfg::dnms_read_Z_active = false;
    cfg::dnms_read_spectrum_active = false;
    cfg::dnms_read_spectrum_Z_active = false;
    cfg::dnms_2nd_interval_active = false;
    cfg::dnms_2nd_interval_Z_active = false;
    cfg::dnms_2nd_read_spectrum_active = false;
    cfg::dnms_2nd_read_spectrum_Z_active = false;
    int_version = int(dnms_version[13] - '0');

    switch (int_version) {
      case 1:
      case 2:
        {
          debug_outln_info(F("DNMS firmware does only support LAeq, LAmin and LAmax"));
          cfg::dnms_read_active = cfg::dnms_read;
          break;
        }

      case 3:
        {
          debug_outln_info(F("DNMS does support LAeq, LAmin, LAmax and frequency spectrum values"));
          cfg::dnms_read_active = cfg::dnms_read;
          cfg::dnms_read_spectrum_active = cfg::dnms_read_spectrum;
          break;
        }

      case 4:
        {
          debug_outln_info(F("DNMS does support LAeq, LAmin, LAmax, frequency spectrum values and 2nd interval"));
          cfg::dnms_read_active = cfg::dnms_read;
          cfg::dnms_read_spectrum_active = cfg::dnms_read_spectrum;
          cfg::dnms_2nd_interval_active = cfg::dnms_2nd_interval;
          cfg::dnms_2nd_read_spectrum_active = cfg::dnms_2nd_read_spectrum;
          break;
        }

      case 5:
        {
          debug_outln_info(F("DNMS does support LAeq, LAmin, LAmax, frequency spectrum values, 2nd interval and Z values"));
          cfg::dnms_read_active = cfg::dnms_read;
          cfg::dnms_read_Z_active = cfg::dnms_read_Z;
          cfg::dnms_read_spectrum_active = cfg::dnms_read_spectrum;
          cfg::dnms_read_spectrum_Z_active = cfg::dnms_read_spectrum_Z;
          cfg::dnms_2nd_interval_active = cfg::dnms_2nd_interval;
          cfg::dnms_2nd_interval_Z_active = cfg::dnms_2nd_interval_Z;
          cfg::dnms_2nd_read_spectrum_active = cfg::dnms_2nd_read_spectrum;
          cfg::dnms_2nd_read_spectrum_Z_active = cfg::dnms_2nd_read_spectrum_Z;
          // set microphone if version is 5.3.x and above
          if(dnms_version[15] >= '3') {
            if(cfg::dnms_ICS43434) {
              if(dnms_set_ICS43434() != 0) {
                debug_outln_error(F("Error setting ICS-43434 microphone!"));
                debug_outln_error(F("Check DNMS wiring"));
                dnms_init_failed = true;
                DNMS_error_count++;
              } else {
                debug_outln_info(F("ICS-43434 microphone set: OK"));
              }
            } else {
              if(cfg::dnms_IM72D128) {
                if(dnms_set_IM72D128() != 0) {
                  debug_outln_error(F("Error setting IM72D128 microphone!"));
                  debug_outln_error(F("Check DNMS wiring"));
                  dnms_init_failed = true;
                  DNMS_error_count++;
                } else {
                  debug_outln_info(F("IM72D128 microphone set: OK"));
                }
              } else {
                // do nothing
              }
      ...
      default:
        {
          debug_outln_info(F("DNMS unkown firmware version, please check!"));
          break;
        }
    }
  }
...
static void powerOnTestSensors() {
  ...
  if (cfg::npm_read) {
    ...
    if (test_state == 0x00) {
      ...
      if (bitRead(test_state, 6) == 1) {
        debug_outln_error(F("Memory error"));
      }
      if (bitRead(test_state, 7) == 1) {
        debug_outln_error(F("Laser error"));
      }
  ...
  if (cfg::sht3x_read) {
    ...
    if (!initSHT3x(0x44) && !initSHT3x(0x45)) {
...
static void logEnabledAPIs() {
  ...
  if (cfg::send2influx) {
   ...
   if (cfg::send2mqtt) {
    debug_outln_info(F("custom MQTT Broker"));
  }
...
static void setupNetworkTime() {
  ...
  settimeofday_cb([]() {
    if (!sntp_time_set) {
      time_t now = time(nullptr);
      debug_outln_info(F("SNTP synced (UTC): "), ctime(&now));
      twoStageOTAUpdate();
  ...
  strcpy_P(ntpServer1, cfg::ntp_1);
  strcpy_P(ntpServer2, cfg::ntp_2);
...
static unsigned long sendDataToOptionalApis(const String &data) {
  ...
  if (send_2nd_dnms) {
    dnms_now = dnms_2nd_now;
      if (cfg::send2mqtt) {
      debug_outln_info(FPSTR(DBG_TXT_SENDING_TO), F("custom MQTT Broker: "));
      RESERVE_STRING(data_4_influxdb, XXXLARGE_STR);
      create_influxdb_string_from_data(data_4_influxdb, data, cfg::measurement_name_mqtt);
      sum_send_time += sendData(LoggerMQTT, data_4_influxdb, 0, cfg::host_mqtt, "");
    }  
    if (cfg::send2influx) {
      debug_outln_info(FPSTR(DBG_TXT_SENDING_TO), F("custom influx db: "));
      RESERVE_STRING(data_4_influxdb, XXXLARGE_STR);
      create_influxdb_string_from_data(data_4_influxdb, data, cfg::measurement_name_influx);
...
void setup(void) {
  ...
  digitalWrite(led_status, LOW);  // turn the LED on (LOW is the voltage level)
  led_status_state = true;
  led_timer = millis();

  Debug.begin(115200);  // Output to Serial at 115200 baud

  cfg::debug_save = cfg::debug;
  cfg::debug = DEBUG_MIN_INFO;
  ...
  Wire.begin(cfg::i2c_gpio_sda, cfg::i2c_gpio_scl);
  Wire.setClock(150000);

  temp_corr_value = float(readCorrectionOffset(cfg::temp_correction));
...
//#define SERIAL_BUFFER_SIZE 256
  ...
  if (cfg::dnms_2nd_switch_output) {
    pinMode(cfg::dnms_2nd_gpio_pin, OUTPUT);  // set GPIO Pin to output for switch funcktion of 2nd DNMS measurement
    digitalWrite(cfg::dnms_2nd_gpio_pin, LOW);
  }
  ...
  if (cfg::send2mqtt) {
    mqtt_client.setClient(wifi_client_for_mqtt);
    mqtt_client.setServer(cfg::host_mqtt, cfg::port_mqtt);
    mqtt_client.setKeepAlive(MQTT_KEEP_ALIVE);
    if (!mqtt_client.setBufferSize(MQTT_BUFFER_SIZE)) {
      debug_outln_info(F("Error setting Buffer Size for MQTT PubSubClient"));
    }
    mqtt_topic += cfg::mqtt_main_topic;
    if (cfg::id_as_mqtt_sub_topic) {
      mqtt_topic += F("/"SENSOR_BASENAME);
      mqtt_topic += esp_chipid;
    }
  }
...
// start on full hour or on full minute
  if (cfg::start_on_full_hour) {
    for (;;) {
      wdt_reset();  // nodemcu is alive
      dnms_now = time(nullptr);
      strcpy(time_string, ctime(&dnms_now));
      if ((time_string[14] == '0') && (time_string[15] == '0')) {
        break;
      }
      delay(10);     
    }
    ...
    if (cfg::start_on_full_minute) {
      for (;;) {
        wdt_reset();  // nodemcu is alive
        dnms_now = time(nullptr);
        strcpy(time_string, ctime(&dnms_now)); 
        if ((time_string[17] == '0') && (time_string[18] == '0')) {        
          break;
        }
        delay(10);
      }
  ...
  dnms_now = time(nullptr);
  debug_outln_info(F("Measurements start(UTC): "), ctime(&dnms_now));
...
void loop(void) {
  ...
  if (msSince(led_timer) > led_status_period_1) {
    led_timer = millis();
    if (led_status_state) {
      digitalWrite(led_status, HIGH);  // turn the LED off (HIGH is the voltage level)
  ...
  if (!sntp_time_set && send_now && msSince(time_point_device_start_ms) < 1000 * 2 * 30 + 5000) {
    debug_outln_error(F("NTP sync not finished yet, skipping send"));
    send_now = false;
    starttime = act_milli;
  }
  ...
  if (cfg::sps30_read && (!sps30_init_failed)) {
    if ((msSince(starttime) - SPS30_read_timer) > SPS30_WAITING_AFTER_LAST_READ) {
      ...
      ++SPS30_read_counter;
      if (ret_SPS30 < 0) {
        debug_outln_error(F("SPS30 error reading measurement"));
        SPS30_read_error_counter++;
      } else {
        if (SPS_IS_ERR_STATE(ret_SPS30)) {
          debug_outln_error(F("SPS30 measurements may not be accurate"));
          SPS30_read_error_counter++;
        }
  ...
  if (cfg::sen5x_read && (!sen5x_init_failed)) {
    if ((msSince(starttime) - SEN5X_read_timer) > SEN5X_WAITING_AFTER_LAST_READ) {
      ...
      error = sen5x.readMeasuredPmValues(massConcentrationPm1p0, massConcentrationPm2p5,
                                         massConcentrationPm4p0, massConcentrationPm10p0, numberConcentrationPm0p5, numberConcentrationPm1p0,
                                         numberConcentrationPm2p5, numberConcentrationPm4p0, numberConcentrationPm10p0, typicalParticleSize);
      if (error) {
  ...
  if ((msSince(last_display_millis) > DISPLAY_UPDATE_INTERVAL_MS) && (cfg::has_display || cfg::has_sh1106 || lcd_1602 || lcd_2004)) {
    display_values();
    last_display_millis = act_milli;
  }

  /* DNMS 2nd measurement interval ? */
  if (cfg::dnms_2nd_interval_active || cfg::dnms_2nd_interval_Z_active || cfg::dnms_2nd_read_spectrum_active || cfg::dnms_2nd_read_spectrum_Z_active) {
    send_dnms_2nd_interval = msSince(starttime_dnms) > cfg::dnms_2nd_interval_time;
    if (send_dnms_2nd_interval) {
      starttime_dnms = millis();
      RESERVE_STRING(data, XXXLARGE_STR);
      data = FPSTR(data_first_part);
      RESERVE_STRING(result, MED_STR);
      fetchSensorDNMS_2nd(result);
      if (cfg::dnms_2nd_interval_active) {
        data += result;
      }
      result = emptyString;

      if (last_value_dnms_laeq_2nd >= cfg::dnms_2nd_threshold) {
        dnms_2nd_count = cfg::dnms_2nd_number_measurements;  // retrigger counter
        if (cfg::dnms_2nd_switch_output) {
          digitalWrite(cfg::dnms_2nd_gpio_pin, HIGH);  // set output pin high
        }
      }

      if (dnms_2nd_count > 0) {
        if (cfg::dnms_2nd_interval_Z_active) {
          fetchSensorDNMS_Z_2nd(result);
          data += result;
          result = emptyString;
        }

        if (cfg::dnms_2nd_read_spectrum_active) {
          fetchSensorDNMS_FFT_2nd(result);
          data += result;
          result = emptyString;
        }

        if (cfg::dnms_2nd_read_spectrum_Z_active) {
          fetchSensorDNMS_FFT_Z_2nd(result);
          data += result;
        }
        ...
        //Debug.println(data);
        send_2nd_dnms = true;
        sendDataToOptionalApis(data);

        dnms_2nd_count--;
        if (dnms_2nd_count == 0) {
          if (cfg::dnms_2nd_switch_output) {
            digitalWrite(cfg::dnms_2nd_gpio_pin, LOW);  // set output to low
          }
        }
      }
    }
  ...
  if (send_now) {
    ...
    if (cfg::scd4x_read && (!scd4x_init_failed)) {
      // getting temperature and humidity (optional)
      fetchSensorSCD4X(result);
      data += result;
      sum_send_time += sendSensorCommunity(result, SCD4X_API_PIN, FPSTR(SENSORS_SCD4X), "SCD4X_");
      something_to_send = true;
      result = emptyString;
    }
    if (cfg::ds18b20_read) {
    ...
    if (cfg::dnms_read_active || cfg::dnms_read_spectrum_active || cfg::dnms_read_Z_active || cfg::dnms_read_spectrum_Z_active) {
    ...
    if (cfg::dnms_read_Z_active) {
      fetchSensorDNMS_Z(result);
      data += result;
      result = emptyString;
    }
    if (cfg::dnms_read_spectrum_active) {
      fetchSensorDNMS_FFT(result);
      data += result;
      result = emptyString;
    }
    if (cfg::dnms_read_spectrum_Z_active) {
      fetchSensorDNMS_FFT_Z(result);
      data += result;
      result = emptyString;
    }
    ...
    //	Debug.println(data);   // print complete JSON string
    ...
    if (WiFi.status() != WL_CONNECTED) {
      debug_outln_error(F("Connection lost, reconnecting "));
      WiFi_error_count++;
      WiFi.reconnect();
      waitForWifiToConnect(20);
    }
    ...
    // cyclic restart configured??
    if (cfg::sensor_cyclic_restart) {
      if (msSince(time_point_device_start_ms) >= (cfg::sensor_cyclic_restart_time * 3600000 - 150000)) {
        sensor_restart();
      }
    }
